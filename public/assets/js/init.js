...
$(function(){
	...
	$('.workForm').submit(function(){
		...
		$.ajax({url:'/send.php',data:formData,type:"POST",enctype: 'multipart/form-data',processData: false,contentType: false,success:function(data){
			if(data.includes('red')){
				$('.workForm .result').html(data);
			}else{
				$('.workForm')[0].reset();
				$('.workForm .result').html('');
				$('.file--upload').html('<label><input type="file" name="doc"><span class="filename">Прикрепить фото работы</span></label>');
	...
	$('.selectFilt').change(function(){
		var order = $(this).find('option:selected').val();
		var cat = $(this).data('id');
		$.ajax({url:'/ajax.php?go=filterUrl',data:'order='+order+'&cat='+cat,type:"POST",success:function(data){
